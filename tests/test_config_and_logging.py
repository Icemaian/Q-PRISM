...
def test_load_yaml_base_has_expected_keys() -> None:
    data = load_yaml(Path(__file__).parent.parent / "src/qprism/configs/base.yaml")
...
def test_load_experiment_config_h2_default() -> None:
    cfg = load_experiment_config(Path(__file__).parent.parent / "src/qprism/configs/experiments/http2_default.yaml")
    ...
    assert cfg.trace_path == Path(Path(__file__).parent.parent / "src/qprism/data/traces/dc_trace.json")
    assert cfg.runs > 0
